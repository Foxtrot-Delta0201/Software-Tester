...
const SUITES: Suite[] = [
  ...
  { key: 'all',             label: 'Full Orchestrator',    desc: 'All categories via orchestrator',       group: 'Meta' },
]
...
export default function TestRunner() {
  ...
  const isPass = (summary?.overall as string) === 'PASS'
    ...
    <div className="p-8 space-y-7 max-w-5xl mx-auto">
      ...
      <div>
        <h1 className="text-2xl font-bold text-white flex items-center gap-2">
          <FlaskConical size={22} className="text-green-400" /> Test Runner
        </h1>
      ...
      <div className="grid lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="glass p-4 space-y-2">
            ...
            <input
              ...
              disabled={running}
              className="w-full rounded-lg bg-slate-800/60 border border-slate-700/50 text-slate-200 px-3 py-2 text-sm font-mono focus:outline-none focus:border-green-500 placeholder-slate-600 disabled:opacity-50"
            />
            <p className="text-xs text-slate-500">Leave blank to use the path set in Settings.</p>
          </div>

          {/* Suite selector */}
          <div className="glass p-4 space-y-3">
            <div className="flex items-center justify-between">
              <p className="text-xs font-medium text-slate-400">Suites ({selected.size} selected)</p>
              <div className="flex gap-2">
                <button onClick={() => setSelected(new Set(SUITES.map(s => s.key)))} className="text-xs text-blue-400 hover:underline">All</button>
              ...
              return (
                <div key={group}>
                  <button
                    ...
                    className="flex items-center gap-1.5 text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5 hover:text-slate-300"
                  >
                    {allOn ? <CheckSquare size={13} className="text-blue-400" /> : <Square size={13} />}
                    {group}
                  </button>
                      ...
                      return (
                        <button
                          ...
                          className={`w-full flex items-center justify-between gap-2 px-3 py-2 rounded-lg text-left transition-colors text-sm ${
                            on ? 'bg-green-900/20 border border-green-700/40' : 'hover:bg-white/5 border border-transparent'
                          } disabled:cursor-default`}
                        >
                          <div className="flex items-center gap-2 min-w-0">
                            <div className={`w-3.5 h-3.5 rounded flex items-center justify-center shrink-0 ${on ? 'bg-green-600' : 'bg-slate-700'}`}>
                              {on && <span className="text-white text-[9px] font-bold">✓</span>}
                            </div>
                            <div className="min-w-0">
                              <p className="font-medium text-slate-200 truncate">{suite.label}</p>
                              <p className="text-xs text-slate-500 truncate">{suite.desc}</p>
                            </div>
                          </div>
        ...
        <div className="lg:col-span-3 space-y-4">
          {/* Summary card */}
          {summary && (
            <div className={`glass p-4 ${isPass ? 'border-green-700/40' : 'border-red-700/40'}`}>
              <p className={`font-semibold text-sm mb-3 ${isPass ? 'text-green-300' : 'text-red-300'}`}>
                Overall: {summary.overall as string} — {summary.elapsed_s as number}s
              </p>
              <div className="grid grid-cols-3 gap-2 text-center">
                <div className="bg-white/5 rounded-lg border border-green-800/30 py-2">
                  <p className="text-xl font-bold text-green-400">{summary.passed as number}</p>
                  <p className="text-xs text-slate-500">Passed</p>
                </div>
                <div className="bg-white/5 rounded-lg border border-red-800/30 py-2">
                  <p className="text-xl font-bold text-red-400">{summary.failed as number}</p>
                  <p className="text-xs text-slate-500">Failed</p>
                </div>
                <div className="bg-white/5 rounded-lg border border-slate-700/30 py-2">
                  <p className="text-xl font-bold text-slate-400">{summary.skipped as number}</p>
